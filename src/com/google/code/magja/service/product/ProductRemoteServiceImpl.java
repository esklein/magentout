...
public class ProductRemoteServiceImpl extends GeneralServiceImpl<Product>
	...
	if (id > 0) {
		...
		// newProduct.add(product.getStoreView()); // TODO: not
		// implemented yet
		...
		if (success) {
		    ...
		    // FIXME: compare new and existing media instead of delete
		    // and create
	...
	for (Map<String, Object> iv : resultList) {
		...
		if (product.getId().equals(
			...
			if (iv.get("is_in_stock").toString().equals("0")
				|| iv.get("is_in_stock").toString().equals(
					"false"))
    ...
    public void setProductTypeConfigurable(Product product)
	    throws ServiceException {
	try {
	    List<Object> params = new LinkedList<Object>();
	    params.add(product.getId());
	    Boolean success = (Boolean) soapClient.call(
		    ResourcePath.ProductSetType, params);
	} catch (AxisFault e) {
	    if (debug)
		e.printStackTrace();
	    throw new ServiceException(e.getMessage());
    ...
    public void setSuperAttribute(Product product, ProductAttribute attribute)
	    throws ServiceException {
	try {
	    List<Object> params = new LinkedList<Object>();
	    params.add(product.getId());
	    ...
	    params.add(0);
	    Boolean success = (Boolean) soapClient.call(
		    ResourcePath.ProductAssignSuperAttribute, params);
	} catch (AxisFault e) {
	    if (debug)
		e.printStackTrace();
	    throw new ServiceException(e.getMessage());
