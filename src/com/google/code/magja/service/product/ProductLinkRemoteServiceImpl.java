...
public class ProductLinkRemoteServiceImpl extends
    ...
    @Override
    public void assignSimpleToConfigurable(Product simpleProduct,
	    Product configurableProduct) throws ServiceException {
	try {			
	    List<Object> params = new LinkedList<Object>();
		params.add(simpleProduct.getId());
		params.add(configurableProduct.getId());
		Boolean success = (Boolean) soapClient.call(ResourcePath.ProductLinkSimpleToConfigurable, params);
	} catch (AxisFault e) {
	    if (debug)
		e.printStackTrace();
	    throw new ServiceException(e.getMessage());
