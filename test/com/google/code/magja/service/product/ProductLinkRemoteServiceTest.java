...
public class ProductLinkRemoteServiceTest {
	...
	@Test
	public void testAssignSimpleToConfigurable() 
	{
	    generateProductsAndSave(2);

		// associate the products

		Product simpleProduct = products.get(0);
		Product configurableProduct = products.get(1);
		simpleProduct.setType(ProductTypeEnum.SIMPLE.getProductType());
		configurableProduct.setType(ProductTypeEnum.CONFIGURABLE.getProductType());

		try {
		    productService.save(simpleProduct);
		    productService.save(configurableProduct);
		...
		try {
			service.assignSimpleToConfigurable(simpleProduct, configurableProduct);
		} catch (ServiceException e) {
			e.printStackTrace();
			fail("Error on save link");
		}
